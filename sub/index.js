...
class RankList{
    ...
    updateRankData() {
        ...
        wx.getUserCloudStorage({
            ...
            success: res => {
                ...
                let tempScore = '-1';
                if(res.KVDataList[0]) {
                    if (res.KVDataList[0].key === 'score')
                        score = res.KVDataList[0].value;
                    else if (res.KVDataList[0].key === 'tempScore')
                        tempScore = res.KVDataList[0].value;
                }
                if(res.KVDataList[1]) {
                    if (res.KVDataList[1].key === 'score')
                        score = res.KVDataList[1].value;
                    else if (res.KVDataList[1].key === 'tempScore')
                        tempScore = res.KVDataList[1].value;
                }                
        ...
        wx.getUserCloudStorage({
            ...
            success: res => {
                if (res.KVDataList[0]) {
                    ...
                    let myScore = res.KVDataList[0].value + ' 分';
    ...
    drawRankList(res) {
        ...
        for (let i in res.data) {
            ...
            let obj = {username: '???', score: '0'};
            if (data.nickname)
                obj.nickname = data.nickname;
            if (data.avatarUrl)
                obj.headimgurl = data.avatarUrl;
            if (data.KVDataList[usernameIndex])
                obj.username = data.KVDataList[usernameIndex].value;
            if (data.KVDataList[scoreIndex])
                obj.score = data.KVDataList[scoreIndex].value;
            rankInfo.push(obj);
        }
        console.log('Sub: rankInfo.length = ' + rankInfo.length);
        // * 数据排序 *
        rankInfo.sort(function (x, y) {
            let intX = parseInt(x.score);
            let intY = parseInt(y.score);
            if (intX > intY)
                return -1;
            if (intX < intY)
                return 1;
        ...
        for (let i = 0; i < RankShowNumber; i++) {
            if (rankInfo[i]) {
                ...
                this.context.fillStyle = '#FFFFFF';
                this.context.textAlign = 'left';
                if (parseInt(rankInfo[i].score) > 9999)
                    this.context.fillText('9999+ 分', 350, 365 + 115 * i); 
                else
                    this.context.fillText([rankInfo[i].score, ' 分'].join(''), 350, 365 + 115 * i); 
            }
            else {
                // 绘制空数据
    ...
    trimString(string, renderLenMax) {
        ...
        for (let i = 0; i < string.length; i++) {
            if (this.isChinese(string.charAt(i)))
                renderLen += 2;
            else
                renderLen += 1;
        }
