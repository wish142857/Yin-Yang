...
cc.Class({
  ...
  update: function (dt) {
    ...
    if (!this.data.fail && !this.bg.noPath) {
      for (let i = this.bg.childrenCount - 1; i >= 0; --i) {
        const childNode = this.bg.children[i]
  ...
  leftShift: function () {
    ...
    if (this.animation.isLeftMoving === false) {
      ...
      if (this.lElementNode.pathNumber === 1) {
        this.lElementNode.pathNumber = 2
        posX = this.data.elementPathLineX_2
      } else {
        this.lElementNode.pathNumber = 1
        posX = this.data.elementPathLineX_1
      }
  ...
  rightShift: function () {
    ...
    if (this.animation.isRightMoving === false) {
      ...
      if (this.rElementNode.pathNumber === 3) {
        ...
        posX = this.data.elementPathLineX_4
      } else {
        this.rElementNode.pathNumber = 3
        posX = this.data.elementPathLineX_3
      }
  ...
  combine: function () {
    ...
    if (this.animation.isLeftMoving === false && this.animation.isRightMoving === false) {
      ...
      if (this.blackLShiftKey.active) {
        this.blackLShiftKey.active = false
        lActive = this.blackLShiftKey
      } else {
        this.whiteLShiftKey.active = false
        lActive = this.whiteLShiftKey
      }
      if (this.blackRShiftKey.active) {
        this.blackRShiftKey.active = false
        rActive = this.blackRShiftKey
      } else {
        this.whiteRShiftKey.active = false
        rActive = this.whiteRShiftKey
      }
      this.bg.noPath = true
      var posX1, posX2
      if (this.lElementNode.pathNumber === 1) {
        posX1 = this.data.elementPathLineX_1
      } else {
        posX1 = this.data.elementPathLineX_2
      }
      if (this.rElementNode.pathNumber === 3) {
        posX2 = this.data.elementPathLineX_3
      } else {
        posX2 = this.data.elementPathLineX_4
      }
  ...
  gameOver: function () {
    ...
    // 结算评价更新
    if (this.data.score >= this.data.scoreA) { this.resultNode.getChildByName('scoreA').active = true } else if (this.data.score >= this.data.scoreB) { this.resultNode.getChildByName('scoreB').active = true } else if (this.data.score >= this.data.scoreC) { this.resultNode.getChildByName('scoreC').active = true } else { this.resultNode.getChildByName('scoreD').active = true }
    // 激活结算界面
  ...
  returnHome: function () {
    // *** 回到主页 ***
    this.combineEffect.active = false
    const musicOn = this.homeShadow.getChildByName('music-on')
    const musicOff = this.homeShadow.getChildByName('music-off')
